...
uint64_t kShadowSize     ;
uint64_t kUnionTableSize ;
uint64_t kAllocationSize ;
...
struct Mapping {
  ...
  static const uptr kShadowMask = (0x100000000000-1);
};
...
ALWAYS_INLINE
uptr HashTableAddr() {
  return UnionTableAddr() + (MappingArchImpl<MAPPING_HASH_TABLE_ADDR>() - MappingArchImpl<MAPPING_UNION_TABLE_ADDR>());
}
